...
- каталог опубликованных проектов и свободных открытых ролей;
- регистрация и вход по логину, email, Google или GitHub;
- обязательное подтверждение email для обычных пользователей;
...
- **Redis** хранит результаты Celery-задач и общие счётчики rate limiting.
...
SQLite хранится в именованном volume `sqlite_data`, общем для Django, Celery
worker и Celery beat. Команда `docker compose down` сохраняет данные, а флаг
`-v` удаляет volumes SQLite, RabbitMQ и статических файлов.

При `DJANGO_SERVE_MEDIA=True` локальный Compose отдаёт avatar, cover и project
files через Django. В production media следует вынести в reverse proxy или
объектное хранилище; файлы проектов должны отдаваться как attachment.

## REST API

API доступен по префиксу `/api/`. Используются собственная Token Authentication
с обязательным подтверждённым email и Session Authentication. Списки разбиваются
на страницы по 20 записей и поддерживают
фильтрацию, поиск через `?search=` и сортировку через `?ordering=`.

Получение токена ограничено пятью попытками в минуту. Создание проектов,
откликов, приглашений и чувствительные POST-действия также ограничены по частоте.
...
GET|PUT|PATCH /api/specialists/me/  # требует существующий профиль
...
[`postman/new_makers_api.postman_collection.json`](postman/new_makers_api.postman_collection.json).

## Подтверждение email

В проекте включено обязательно подтверждение email для обычных пользователей.

После регистрации пользователь получает письмо со ссылкой подтверждения и
приветственное письмо. До подтверждения email пользователь не может войти через
HTML-форму или получить API-токен.
...
Email-задачи повторяются при временных SMTP-сбоях с exponential backoff. Digest
разбивается на отдельные задачи для каждого получателя, поэтому ошибка одного
адреса не останавливает остальных.
...
2. Настроить HTTPS и reverse proxy, например Nginx, перед Gunicorn.
   Reverse proxy должен обслуживать `MEDIA_ROOT`; project files следует отдавать
   с `Content-Disposition: attachment` и `X-Content-Type-Options: nosniff`.
